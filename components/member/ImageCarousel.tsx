...
export function ImageCarousel({ images }: ImageCarouselProps) {
  ...
  return (
    <>
      ...
      <button type="button" aria-label="Previous photo" onClick={() => scrollToImage(activeIndex - 1)} disabled={activeIndex === 0} className="absolute left-3 top-1/2 hidden h-11 w-11 -translate-y-1/2 place-items-center rounded-full bg-white/95 text-2xl font-bold text-kasset-ink shadow-md transition hover:text-kasset-green disabled:cursor-not-allowed disabled:opacity-50 md:grid">‹</button>
      <button type="button" aria-label="Next photo" onClick={() => scrollToImage(activeIndex + 1)} disabled={activeIndex === images.length - 1} className="absolute right-3 top-1/2 hidden h-11 w-11 -translate-y-1/2 place-items-center rounded-full bg-white/95 text-2xl font-bold text-kasset-ink shadow-md transition hover:text-kasset-green disabled:cursor-not-allowed disabled:opacity-50 md:grid">›</button>
      <div className="absolute bottom-4 left-1/2 flex -translate-x-1/2 gap-1.5 rounded-full bg-white/80 px-2 py-1 backdrop-blur" aria-label="Photo indicators">
        {images.map((item, index) => (
          <button key={`${item}-${index}`} type="button" aria-label={`View photo ${index + 1}`} onClick={() => scrollToImage(index)} className={`h-2 w-2 rounded-full ${index === activeIndex ? "bg-kasset-green" : "bg-slate-300"}`} />
        ))}
      </div>
    </>
  );
