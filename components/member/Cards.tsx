...
export function StatusBadge({ status }: { status: string }) {
  return (
    <span className="inline-flex items-center rounded-md border border-slate-200 bg-slate-50 px-3 py-1 text-[0.7rem] font-semibold uppercase tracking-wide text-kasset-green">
      {status}
    </span>
  );
}
...
export function MetricCard({ label, value, helper, trend }: { label: string; value: string; helper: string; trend?: string }) {
  return (
    <article className="min-w-0 rounded-2xl border border-slate-200/60 bg-white/85 p-4 shadow-[0_1px_2px_rgba(15,23,42,0.03)] transition hover:border-slate-300/80 sm:p-5">
      ...
      <p className="mt-3 text-xl font-semibold tracking-[-0.04em] text-kasset-ink sm:text-3xl">{value}</p>
      <p className="mt-2 text-[0.72rem] leading-5 text-slate-500 sm:text-sm">{helper}</p>
      {trend ? <p className="mt-4 inline-flex rounded-full bg-emerald-50 px-2 py-1 text-[0.68rem] font-semibold text-kasset-green">{trend}</p> : null}
    </article>
  );
}
...
export function ProgressBar({ value }: { value: number }) {
  return (
    <div className="h-2.5 overflow-hidden rounded-md bg-slate-100">
      <div className="h-full rounded-md bg-kasset-green" style={{ width: `${Math.min(value, 100)}%` }} />
    </div>
  );
}

function participantCountLabel(count: number) {
  return count === 1 ? "1 participant" : `${count.toLocaleString()} participants`;
}
...
export function OpportunityCard({ opportunity }: { opportunity: Opportunity }) {
  ...
  return (
    <article className="overflow-hidden rounded-xl border border-slate-200/80 bg-white shadow-[0_1px_2px_rgba(15,23,42,0.03)] transition hover:border-slate-300">
      <PendingLink href={detailHref} pendingLabel="Opening..." className="group block">
        <div className={`relative grid h-44 place-items-center sm:h-48 ${opportunity.imageUrl ? "bg-cover bg-center" : "bg-slate-100"}`} style={opportunity.imageUrl ? { backgroundImage: `url(${opportunity.imageUrl})` } : undefined}>
          ...
          <span className="absolute left-3 top-3 rounded-md bg-white/95 px-3 py-1 text-[0.7rem] font-semibold uppercase tracking-wide text-kasset-green">{statusLabel}</span>
        </div>
      </PendingLink>
      <div className="space-y-4 p-5">
        <div>
          ...
          <PendingLink href={detailHref} pendingLabel="Opening..." className="group">
            <h3 className="text-lg font-semibold tracking-tight text-kasset-ink group-hover:text-kasset-green">{opportunity.title}</h3>
          </PendingLink>
        ...
        <div>
          ...
          <div className="mt-3 flex items-center justify-between gap-3 text-sm text-slate-600">
            ...
            <p className="whitespace-nowrap text-xs text-slate-500">{participantCountLabel(opportunity.participants)}</p>
          </div>
        </div>
        ...
        <PendingLink href={detailHref} pendingLabel="Opening..." className="inline-flex min-h-10 w-full items-center justify-center rounded-md bg-kasset-green px-4 py-2 text-sm font-semibold text-white transition hover:bg-kasset-green/90">
