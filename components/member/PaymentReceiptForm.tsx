...
function SubmitButton({ resubmission }: { resubmission: boolean }) {
  ...
  return (
    <button type="submit" disabled={pending} className="inline-flex min-h-11 w-full items-center justify-center rounded-xl bg-papaipay-green px-4 py-2.5 text-sm font-bold text-white transition hover:bg-papaipay-ink disabled:cursor-not-allowed disabled:opacity-60">
  ...
  return (
    <form action={formAction} className="mt-4 space-y-4">
      ...
      <label className="block">
        ...
        <input name="bankReference" required defaultValue={existingReference ?? ""} placeholder="Example: bank transaction reference" className="mt-1 min-h-11 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm outline-none focus:border-papaipay-green" />
      </label>
      <label className="block rounded-xl border border-dashed border-slate-300 bg-white p-4">
        <span className="block text-sm font-bold text-papaipay-ink">Receipt file</span>
        <span className="mt-1 block text-xs leading-5 text-slate-500">JPG, PNG or PDF, up to 5MB.</span>
        <input name="receipt" type="file" accept="image/jpeg,image/png,application/pdf" required className="mt-3 block w-full min-w-0 text-sm text-slate-600 file:mr-3 file:rounded-lg file:border-0 file:bg-emerald-50 file:px-3 file:py-2 file:text-xs file:font-bold file:text-papaipay-green" />
      </label>
      <label className="block">
        <span className="text-xs font-bold uppercase tracking-wide text-slate-500">Note (optional)</span>
        <textarea name="notes" rows={2} placeholder="Add any information for the admin reviewer." className="mt-1 w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm outline-none focus:border-papaipay-green" />
      </label>
