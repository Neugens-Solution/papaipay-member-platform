...
function SubmitButton() {
  ...
  return (
    <button disabled={pending} className="mt-5 min-h-12 w-full rounded-xl bg-kasset-green px-5 py-3 text-sm font-bold text-white shadow-[0_10px_24px_rgba(34,139,76,0.24)] transition hover:bg-kasset-green/90 disabled:cursor-not-allowed disabled:opacity-70">
  ...
  return (
    <form action={formAction}>
      ...
      <div className="mt-4 divide-y divide-slate-100 rounded-xl bg-slate-50/70 px-3">
        <div className="flex items-start justify-between gap-4 border-b border-slate-100 py-3"><dt className="text-sm text-slate-500">Minimum Participation Amount</dt><dd className="text-right text-sm font-bold text-kasset-ink">{minimumParticipation}</dd></div>
        <div className="flex items-start justify-between gap-4 border-b border-slate-100 py-3"><dt className="text-sm text-slate-500">Maximum Participation Amount</dt><dd className="text-right text-sm font-bold text-kasset-ink">{maximumParticipation}</dd></div>
        <div className="flex items-start justify-between gap-4 border-b border-slate-100 py-3"><dt className="text-sm text-slate-500">Listing Target</dt><dd className="text-right text-sm font-bold text-kasset-ink">{targetAmount}</dd></div>
        <div className="flex items-start justify-between gap-4 py-3"><dt className="text-sm text-slate-500">Available to Reserve</dt><dd className="text-right text-sm font-bold text-kasset-ink">{remainingAmount}</dd></div>
      </div>
      <SubmitButton />
