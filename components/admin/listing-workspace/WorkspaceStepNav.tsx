...
function SaveStepButton({
  ...
  return (
    <button
      ...
      value={intent}
      disabled={pending}
      className={`rounded-md px-4 py-2 text-sm font-black transition disabled:cursor-not-allowed disabled:opacity-60 ${isSaved ? "border border-emerald-100 bg-emerald-50 text-kasset-green" : "border border-slate-200 bg-white text-slate-700 hover:border-emerald-200 hover:text-kasset-green"}`}
    >
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between gap-3 text-[0.68rem] font-black uppercase tracking-[0.16em] text-slate-400">
        ...
        <span className="text-kasset-green">Scroll →</span>
      </div>
      <div
        ...
        <div className="flex w-max gap-2 pr-6">
          {steps.map((step, index) => (
            <button
              ...
              className={`min-w-[8rem] rounded-2xl border px-3 py-2.5 text-left transition sm:min-w-[9rem] ${activeStep === index ? "border-kasset-green bg-emerald-50 shadow-sm" : "border-slate-200 bg-white hover:border-emerald-100"}`}
            >
              <span className="block text-xs font-black text-kasset-ink">
                {index + 1}. {step.label}
              </span>
              <WorkspaceStatusBadge
                status={
