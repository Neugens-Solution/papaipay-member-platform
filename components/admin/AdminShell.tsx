  ...
  return (
    <div className="min-h-screen bg-[#f6f3ed] text-[#25282d]">
      <div className="lg:flex lg:h-screen lg:overflow-hidden">
        <aside className="hidden border-r border-[#172235]/15 bg-[#fffdf9]/95 lg:sticky lg:top-0 lg:block lg:h-screen lg:w-60 lg:flex-none">
          <div className="px-5 py-6">
            <Link href="/admin/dashboard" className="block rounded-md focus:outline-none focus:ring-2 focus:ring-[#a47c48]">
              ...
              />
              <p className="mt-3 text-xs font-semibold uppercase tracking-[0.18em] text-[#a47c48]">Admin Portal</p>
            </Link>
          </div>
          ...
          <header className="sticky top-0 z-20 border-b border-[#172235]/15 bg-[#fffdf9]/95 px-4 py-3 backdrop-blur sm:px-6 lg:px-8">
            <div className="flex items-center justify-between gap-3">
              <Link href="/admin/dashboard" className="rounded-md focus:outline-none focus:ring-2 focus:ring-[#a47c48] lg:hidden">
                ...
                />
                <p className="mt-1 text-[0.62rem] font-bold uppercase tracking-[0.18em] text-[#a47c48]">Admin Portal</p>
              </Link>
              <div className="ml-auto flex items-center gap-2">
                <details className="group relative">
                  ...
                  <div className="absolute right-0 mt-2 w-60 rounded-lg border border-[#172235]/15 bg-white p-2 shadow-sm">
                    ...
                    <Link href="/admin/profile" className="block rounded-md px-3 py-2.5 text-sm font-semibold text-slate-600 hover:bg-[#f7efe3] hover:text-[#a47c48]">Admin Profile</Link>
                    <Link href="/logout" className="block rounded-md px-3 py-2.5 text-sm font-semibold text-slate-600 hover:bg-[#f7efe3] hover:text-[#a47c48]">Logout</Link>
                  </div>
                </details>
              </div>
            </div>
      ...
      <nav className="fixed inset-x-0 bottom-0 z-30 border-t border-[#172235]/15 bg-[#fffdf9]/95 px-2 pb-[env(safe-area-inset-bottom)] shadow-sm backdrop-blur lg:hidden" aria-label="Admin mobile navigation">
          ...
          {primaryMobileNavItems.map(([label, href]) => {
            ...
            return (
              <Link
                ...
                href={href}
                aria-current={active ? "page" : undefined}
                className={`flex min-h-12 min-w-0 items-center justify-center rounded-xl px-1 text-center text-[0.62rem] font-semibold transition ${active ? "bg-[#f4eadc] text-[#172235]" : "text-slate-500 hover:bg-[#f7efe3] hover:text-[#a47c48]"}`}
              >
