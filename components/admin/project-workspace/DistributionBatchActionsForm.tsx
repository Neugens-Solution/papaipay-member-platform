  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap gap-3">
        <form action={saveAction}><input type="hidden" name="campaignId" value={campaignId} /><input type="hidden" name="settlementId" value={settlementId ?? ""} /><SubmitButton disabled={saveDisabled || Boolean(completed)} label={draftSaved ? "Draft Batch Saved" : "Save Draft Batch"} pendingLabel="Saving Draft…" activeClass="rounded-lg border border-kasset-green bg-kasset-green px-4 py-2.5 text-sm font-bold text-white transition hover:bg-kasset-green/90" /></form>
      ...
      {approved ? (
        <form action={markPaidAction} className="space-y-4 rounded-2xl border border-amber-200 bg-amber-50/70 p-5">
          ...
          <div><h3 className="text-base font-bold text-kasset-ink">Record Manual Payment</h3><p className="mt-1 text-sm leading-6 text-slate-600">Use this only after finance has completed the manual transfer outside the platform. This action records the payment outcome and does not execute a transfer.</p></div>
          <div className="grid gap-4 md:grid-cols-2">
            <label className="block text-xs font-bold uppercase tracking-wide text-slate-500">Payment Date<input name="paymentDate" type="date" required className="mt-1 min-h-10 w-full rounded-lg border border-slate-200 bg-white px-3 text-sm font-semibold outline-none focus:border-kasset-green" /></label>
          ...
          <label className="flex gap-3 rounded-xl border border-amber-200 bg-white p-3 text-sm font-semibold leading-6 text-slate-700"><input name="confirmation" type="checkbox" required className="mt-1 h-4 w-4 rounded border-slate-300 text-kasset-green" />I confirm the manual transfer/payment has already been completed outside the platform. This action only records the outcome and does not execute a transfer.</label>
      ...
      {saveState.status === "success" ? <p className="rounded-xl border border-emerald-100 bg-emerald-50 p-3 text-sm font-semibold text-kasset-green">{saveState.message}</p> : null}
      ...
      {approveState.status === "success" ? <p className="rounded-xl border border-emerald-100 bg-emerald-50 p-3 text-sm font-semibold text-kasset-green">{approveState.message}</p> : null}
      ...
      {markPaidState.status === "success" ? <p className="rounded-xl border border-emerald-100 bg-emerald-50 p-3 text-sm font-semibold text-kasset-green">{markPaidState.message}</p> : null}
