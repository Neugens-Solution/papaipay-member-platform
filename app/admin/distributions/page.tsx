...
export default async function DistributionsPage() {
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6">
      ...
      <div className="rounded-xl border border-emerald-100 bg-emerald-50/60 px-4 py-3 text-sm text-slate-600">Distribution payments are recorded from each project workspace after finance completes the transfer outside the platform.</div>
      <TableWrap><thead><tr><Th>Distribution</Th><Th>Project</Th><Th>Member</Th><Th>Final Total</Th><Th>Status</Th><Th>Payment</Th><Th>Action</Th></tr></thead><tbody>{distributions.map((distribution) => <tr key={distribution.id} className="border-t border-slate-100 align-top"><Td><span className="font-bold text-kasset-ink">{distribution.distributionRef}</span><span className="block text-xs text-slate-400">Batch {distribution.distributionBatch.batchRef}</span></Td><Td><span className="font-bold text-kasset-ink">{distribution.campaign.title}</span><span className="block text-xs text-slate-400">{distribution.campaign.campaignCode}</span></Td><Td><span className="font-bold text-kasset-ink">{distribution.member.fullName}</span><span className="block text-xs text-slate-400">{distribution.member.memberRef}</span></Td><Td>{formatCurrency(decimalToNumber(distribution.finalDistributionTotal))}</Td><Td><Badge>{formatEnumLabel(String(distribution.status))}</Badge></Td><Td><span className="block">{distribution.paymentDate ? formatDate(distribution.paymentDate) : "Not paid"}</span><span className="block text-xs text-slate-400">{distribution.paymentReference || "No reference"}</span></Td><Td><Link className="font-bold text-kasset-green" href={`/admin/distributions/${distribution.id}`}>View details →</Link></Td></tr>)}</tbody></TableWrap>
