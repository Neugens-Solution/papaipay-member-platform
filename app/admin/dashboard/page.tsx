  ...
  return (
    <div className="mx-auto max-w-7xl space-y-7">
        ...
        <div className="mt-5 grid gap-3 md:grid-cols-3">
          {priorities.map((item) => (
            <Link key={item.label} href={item.href} className={`rounded-2xl border p-4 transition hover:-translate-y-0.5 hover:shadow-sm ${item.tone === "amber" ? "border-amber-200 bg-amber-50/70" : item.tone === "green" ? "border-emerald-200 bg-emerald-50/70" : "border-slate-200 bg-slate-50"}`}>
              <p className="text-3xl font-semibold tracking-tight text-kasset-ink">{item.value}</p>
              <p className="mt-2 text-sm font-bold text-slate-700">{item.label}</p>
              <span className="mt-4 inline-flex text-xs font-bold text-kasset-green">Open queue →</span>
            </Link>
          ))}
        </div>
      </Card>
      ...
      <section className="grid min-w-0 gap-4 xl:grid-cols-2">
        <Card>
          <div className="flex items-center justify-between"><h2 className="text-lg font-bold tracking-tight">Recent Listings</h2><Link href="/admin/listings" className="text-sm font-bold text-kasset-green">View all</Link></div>
        ...
        <Card>
          <div className="flex items-center justify-between"><h2 className="text-lg font-bold tracking-tight">Recent Members</h2><Link href="/admin/members" className="text-sm font-bold text-kasset-green">View all</Link></div>
          <div className="mt-5 divide-y divide-slate-100">
            {data.recentMembers.map((member) => (
              <Link key={member.id} href={`/admin/members/${member.id}`} className="flex min-w-0 items-center justify-between gap-4 py-4 first:pt-0 last:pb-0">
                <div className="min-w-0"><p className="truncate font-bold text-kasset-ink">{member.fullName}</p><p className="mt-1 truncate text-xs text-slate-500">{member.memberRef} • {member.user.email}</p><p className="mt-1 text-xs text-slate-400">Joined {formatDate(member.createdAt)}</p></div>
