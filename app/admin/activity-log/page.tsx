  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6">
      ...
      <form className="grid gap-3 rounded-2xl border border-slate-200 bg-white p-4 sm:grid-cols-[1fr_220px_auto]">
        <input name="q" defaultValue={filters?.q || ""} className="min-h-11 min-w-0 rounded-xl border border-slate-200 px-3 text-sm outline-none focus:border-papaipay-green" placeholder="Search action, reference, ID or user" />
        <select name="entity" defaultValue={filters?.entity || ""} className="min-h-11 rounded-xl border border-slate-200 px-3 text-sm font-semibold text-slate-600 outline-none focus:border-papaipay-green"><option value="">All record types</option>{entityTypes.map((type) => <option key={type} value={type}>{formatEnumLabel(type)}</option>)}</select>
        <button type="submit" className="min-h-11 rounded-xl bg-papaipay-green px-5 text-sm font-bold text-white">Filter</button>
      </form>
      <TableWrap>
