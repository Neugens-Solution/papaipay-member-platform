...
function SubmitButton() { const { pending } = useFormStatus(); return <button disabled={pending} className="mt-6 min-h-12 w-full rounded-xl bg-papaipay-ink px-5 py-3 text-sm font-bold text-white shadow-sm transition hover:bg-slate-800 disabled:opacity-70">{pending ? "Signing in..." : "Sign in as admin"}</button>; }
