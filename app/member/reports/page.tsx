      ...
      <section className="grid gap-3 sm:grid-cols-3">
        {categories.map((category) => (
          <div key={category} className="rounded-lg border border-slate-200 bg-white p-4">
            <p className="text-sm font-bold text-kasset-ink">{category}</p>
          </div>
        ))}
      </section>
        ...
        <div className="divide-y divide-slate-100">
          {reportRecords.map((report) => (
            <div key={report.slug} className="grid gap-3 px-4 py-4 sm:px-5 lg:grid-cols-[1.4fr_0.8fr_0.9fr_0.8fr_auto_auto] lg:items-center">
              <div>
                <p className="text-sm font-bold text-kasset-ink">{report.reportName}</p>
              ...
              <button className="inline-flex min-h-10 items-center justify-center rounded-md border border-kasset-green px-4 py-2 text-sm font-bold text-kasset-green transition hover:bg-kasset-green hover:text-white">View</button>
              <button className="min-h-10 rounded-md bg-kasset-green px-4 py-2 text-sm font-bold text-white">Download</button>
            </div>
          ))}
        </div>
      </ContentCard>
