    ...
    <div className="mx-auto w-full max-w-7xl space-y-8 sm:space-y-10">
      <section className="overflow-hidden rounded-[2rem] border border-slate-200/70 bg-white p-6 shadow-[0_20px_70px_rgba(15,23,42,0.06)] sm:p-8 lg:p-10">
        <div className="max-w-3xl">
          <p className="text-sm font-semibold text-kasset-green">Welcome, {firstName}</p>
          <h1 className="mt-3 text-3xl font-semibold tracking-[-0.045em] text-kasset-ink sm:text-5xl">Welcome back.</h1>
      ...
      <div className="grid gap-6 xl:grid-cols-[0.95fr_1.4fr]">
        <section className="rounded-2xl border border-slate-200/70 bg-white p-5 sm:p-6" aria-label="Portfolio allocation">
          <div className="flex items-center justify-between gap-4">
            <div>
              ...
              <h2 className="mt-2 text-xl font-semibold tracking-tight text-kasset-ink">By asset category</h2>
            </div>
          </div>
        ...
        <section className="rounded-2xl border border-slate-200/70 bg-white p-5 sm:p-6" aria-label="My opportunities">
          <div className="flex items-center justify-between gap-4">
            <div><p className="text-xs font-bold uppercase tracking-[0.18em] text-slate-400">My Opportunities</p><h2 className="mt-2 text-xl font-semibold tracking-tight text-kasset-ink">Participated opportunities</h2></div>
            <Link href="/member/portfolio" className="text-sm font-bold text-kasset-green">View all</Link>
          </div>
          <div className="mt-6 grid gap-4 lg:grid-cols-2">
            {portfolioRecords.length === 0 ? <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-8 text-center"><h3 className="font-semibold">Your portfolio is ready.</h3><p className="mt-2 text-sm text-slate-500">Browse opportunities to submit your first participation.</p></div> : portfolioRecords.slice(0, 4).map((record) => (
              <article key={record.slug} className="rounded-2xl border border-slate-200 bg-slate-50/60 p-4">
                <div className="flex items-start justify-between gap-3"><div><p className="text-xs font-bold text-slate-400">{getAssetCategory(record)}</p><h3 className="mt-1 font-semibold text-kasset-ink">{record.propertyName}</h3></div><StatusBadge status={String(record.status)} /></div>
                <dl className="mt-4 grid grid-cols-2 gap-3 text-sm"><div><dt className="text-slate-500">Participation</dt><dd className="font-semibold">{formatRMCompact(record.participationAmount)}</dd></div><div><dt className="text-slate-500">Projected Holding Return</dt><dd className="font-semibold">{record.estimatedYield}</dd></div><div><dt className="text-slate-500">Submitted</dt><dd className="font-semibold">{record.dateSubmitted}</dd></div></dl>
                <Link href={record.slug.startsWith("par-") || record.slug.length > 20 ? `/member/participations/${record.slug}` : `/member/portfolio/${record.slug}`} className="mt-4 inline-flex text-sm font-bold text-kasset-green">View Details →</Link>
              </article>
            ))}
          </div>
        </section>
