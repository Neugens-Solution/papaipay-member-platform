...
function Summary({ opportunity }: { opportunity: NonNullable<Awaited<ReturnType<typeof getRealMemberCampaignBySlug>>> }) {
  return <ContentCard><h2 className="text-lg font-bold">Opportunity Summary</h2><dl className="mt-4 divide-y divide-slate-100 text-sm"><Row label="Opportunity" value={opportunity.title} /><Row label="Location" value={opportunity.location} /><Row label="Resale Price" value={opportunity.resalePrice > 0 ? formatRM(opportunity.resalePrice) : "To be confirmed"} /><Row label="Projected Holding Return" value={opportunity.estimatedYield} /><Row label="Asset Category" value={opportunity.assetCategory} /><Row label="Occupancy Status" value={opportunity.occupancyStatus} /></dl></ContentCard>;
}
