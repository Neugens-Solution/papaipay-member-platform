...
export default function OpportunityDetailPage({ params }: { params: { slug: string } }) {
  ...
  return (
    <div className="space-y-8">
      <section className="overflow-hidden rounded-3xl border border-slate-200/80 bg-white shadow-[0_14px_40px_rgba(15,23,42,0.06)]">
      ...
      <section className="grid gap-6 xl:grid-cols-[1.5fr_.9fr]">
        ...
        <aside className="space-y-6 xl:sticky xl:top-28 xl:self-start">
          ...
          <ContentCard className="border-papaipay-ink bg-papaipay-ink text-white"><h3 className="text-2xl font-bold">Participate Section</h3><p className="mt-3 text-white/75">Mock participation call-to-action only. No payments, backend, authentication, or API routes are created.</p><button className="mt-5 min-h-12 w-full rounded-2xl bg-white px-5 py-3 font-bold text-papaipay-ink transition hover:bg-emerald-50">Review Participation Interest</button></ContentCard>
        </aside>
      </section>
    </div>
  );
