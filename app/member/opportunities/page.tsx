    ...
    <div className="space-y-6">
      <header className="space-y-4">
        ...
        <div className="flex gap-2 overflow-x-auto pb-1" aria-label="Listing tabs">
          {['Open', 'Completed', 'All'].map((tab, index) => <button key={tab} className={`rounded-full px-4 py-2 text-sm font-bold ${index === 0 ? 'bg-papaipay-green text-white' : 'border border-slate-200 bg-white text-slate-600'}`}>{tab}</button>)}
        </div>
      ...
      <section className="grid gap-5 xl:grid-cols-3">
        {opportunities.map((campaign) => <OpportunityCard key={campaign.id} opportunity={campaign} />)}
      </section>
          ...
          {completedCampaigns.map((campaign) => (
            <Link key={campaign.slug} href={`/member/opportunities/${campaign.slug}/outcome`} className="rounded-2xl border border-slate-200 bg-white p-5 transition hover:border-papaipay-green/40">
              ...
              <dl className="mt-4 grid grid-cols-2 gap-3 text-sm">
                ...
                <Summary label="Purchase Price" value={formatRM(campaign.purchasePrice)} />
