...
const inputClass = "min-h-11 min-w-0 rounded-xl border border-slate-200/80 bg-white px-4 py-3 text-sm outline-none transition focus:border-kasset-green/50 focus:ring-4 focus:ring-kasset-green/10";
  ...
  return (
    <div className="space-y-6">
      <header className="space-y-4">
        <div>
          <p className="text-xs font-bold uppercase tracking-[0.22em] text-kasset-green">Member Portal</p>
          <h1 className="mt-2 text-2xl font-semibold tracking-[-0.03em] text-kasset-ink sm:text-3xl">Opportunities</h1>
        ...
        <nav className="flex gap-2 overflow-x-auto pb-1" aria-label="Opportunity tabs">
          {tabs.map((tab) => {
            ...
            return <PendingLink key={tab} href={`/member/opportunities?tab=${tab}`} pendingLabel="Loading…" className={`rounded-full px-4 py-2 text-sm font-bold ${active ? "bg-kasset-green text-white" : "border border-slate-200 bg-white text-slate-600"}`}>{tab.replace(/^./, (character) => character.toUpperCase())}</PendingLink>;
          })}
        </nav>
        <form method="get" className="sticky top-[65px] z-10 -mx-4 grid gap-2 border-y border-slate-200/70 bg-[#f7f8f5]/95 px-4 py-3 backdrop-blur sm:top-[73px] sm:mx-0 sm:rounded-2xl sm:border sm:bg-white/90 sm:p-3 sm:shadow-[0_1px_2px_rgba(15,23,42,0.03)] md:grid-cols-[1.5fr_1fr_1fr_auto_auto]">
          ...
          <button type="submit" className="min-h-11 rounded-xl bg-kasset-green px-5 text-sm font-bold text-white">Apply</button>
...
function CampaignGrid({ title, campaigns, empty }: { title: string; campaigns: Awaited<ReturnType<typeof getMemberCampaignSummaries>>; empty: string }) {
  return (
    <section className="space-y-4">
      <h2 className="text-xl font-bold text-kasset-ink">{title}</h2>
