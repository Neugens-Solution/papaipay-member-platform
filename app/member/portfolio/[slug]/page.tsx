    ...
    <div className="space-y-5">
      ...
      <section className="grid gap-5 xl:grid-cols-[1.3fr_.7fr]">
        ...
        <aside className="space-y-5">
          <ContentCard><h2 className="text-lg font-bold">Distribution Summary</h2><div className="mt-4 space-y-3"><Info label="Distribution ID" value={record.distributionId} /><Info label="Distribution Batch" value={record.distributionBatchId} /><Info label="Payment Reference" value={record.paymentReference} /><Info label="Status" value={record.distributionStatus} /><Info label="Principal Return" value={formatRM(record.principalReturn)} /><Info label="Holding Return" value={formatRM(record.holdingReturn)} /><Info label="Return Distribution" value={formatRM(record.profitDistribution)} /><Info label="Final Return Total" value={formatRM(record.finalDistributionTotal)} /><Info label="Paid Date" value={record.paidDate ?? "Pending"} /><Info label="Admin Notes" value={record.adminNotes} /></div></ContentCard>
          <ContentCard><h2 className="text-lg font-bold">Property Summary</h2><div className="mt-4 grid gap-3"><Info label="Property Type" value={record.property.propertyType} /><Info label="Tenure" value={record.property.tenure} /><Info label="Bumi Status" value={record.property.bumiStatus} /><Info label="Participation Target" value={formatRM(record.property.targetAmount)} /><Info label="Participated Amount" value={formatRM(record.property.collectedAmount)} /><Info label="Current Members" value={`${record.property.currentParticipants}`} /></div><Link href="/member/opportunities" className="mt-5 inline-flex min-h-11 w-full items-center justify-center rounded-md bg-papaipay-green px-4 py-2 text-sm font-bold text-white hover:bg-papaipay-green/90">View Original Listing</Link><Link href="/member/portfolio" className="mt-3 inline-flex min-h-11 w-full items-center justify-center rounded-md border border-slate-200 px-4 py-2 text-sm font-bold text-slate-700 hover:border-papaipay-green hover:text-papaipay-green">Back to Portfolio</Link></ContentCard>
        </aside>
      </section>
    </div>
  );
