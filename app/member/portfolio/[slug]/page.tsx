    ...
    <div className="space-y-5">
      ...
      <header className="rounded-lg border border-slate-200 bg-white p-5">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div><p className="text-xs font-bold text-slate-400">{record.campaignId} • {record.participationId}</p><h1 className="text-2xl font-bold tracking-tight sm:text-[1.7rem]">{record.propertyName}</h1><p className="mt-1 text-sm text-slate-500">{record.location}</p></div>
      ...
      <section className="grid gap-5 xl:grid-cols-[1.3fr_.7fr]">
        ...
        <aside className="space-y-5">
          <ContentCard><h2 className="text-lg font-bold">Distribution Summary</h2><div className="mt-4 space-y-3"><Info label="Distribution ID" value={record.distributionId} /><Info label="Payment Reference" value={record.paymentReference} /><Info label="Status" value={record.distributionStatus} /><Info label="Principal Return" value={formatRM(record.principalReturn)} /><Info label="Holding Return" value={formatRM(record.holdingReturn)} /><Info label="Profit Distribution" value={formatRM(record.profitDistribution)} /><Info label="Final Distribution Total" value={formatRM(record.finalDistributionTotal)} /><Info label="Paid Date" value={record.paidDate ?? "Pending"} /><Info label="Admin Notes" value={record.adminNotes} /></div></ContentCard>
