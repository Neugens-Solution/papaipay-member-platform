...
const filters = ["All", "Campaign Opened", "Active", "Processing", "Completed"];
        ...
        {portfolioRecords.map((record) => (
          <article key={record.slug} className="rounded-lg border border-slate-200 bg-white p-4 sm:p-5">
            <div className="grid gap-4 lg:grid-cols-[1.4fr_2fr_auto] lg:items-center">
              <div><h2 className="font-bold text-papaipay-ink">{record.propertyName}</h2><p className="mt-1 text-sm text-slate-500">{record.location}</p></div>
              <dl className="grid grid-cols-2 gap-3 text-sm xl:grid-cols-4">
                ...
                <div><dt className="text-xs font-bold uppercase text-slate-400">Distribution</dt><dd className="mt-1 font-bold text-slate-800">{record.distributionStatus === "Completed" ? formatRM(record.distributionReceived ?? 0) : record.distributionStatus}</dd>{record.paidDate ? <p className="mt-1 text-xs text-slate-500">Paid Date {record.paidDate}</p> : null}</div>
              </dl>
