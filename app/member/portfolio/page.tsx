  ...
  return (
    <div className="mx-auto w-full max-w-7xl space-y-8">
      <header className="rounded-[2rem] border border-slate-200/70 bg-white p-6 sm:p-8">
        <p className="text-xs font-bold uppercase tracking-[0.22em] text-kasset-green">Portfolio</p>
        <div className="mt-3 max-w-3xl"><h1 className="text-3xl font-semibold tracking-[-0.045em] text-kasset-ink sm:text-5xl">Your participation portfolio.</h1><p className="mt-4 text-base leading-7 text-slate-500">A consolidated view of participation amount, status, projected holding return details, property category, occupancy and submission history.</p></div>
      </header>
      ...
      <nav className="flex gap-2 overflow-x-auto pb-1" aria-label="Portfolio filters">
        {filters.map((filter) => <button key={filter} className="whitespace-nowrap rounded-full border border-slate-200 bg-white px-4 py-2 text-xs font-bold text-slate-600 first:border-kasset-green first:bg-kasset-green first:text-white hover:border-kasset-green/40">{filter}</button>)}
      </nav>

      <section className="grid gap-4 lg:grid-cols-2" aria-label="Portfolio records">
        {portfolioRecords.length === 0 ? (
          <div className="rounded-[2rem] border border-dashed border-slate-300 bg-white p-10 text-center lg:col-span-2"><h2 className="text-xl font-semibold text-kasset-ink">No participation records yet.</h2><p className="mt-2 text-sm text-slate-500">Browse opportunities and submit a participation to begin building your portfolio.</p><Link href="/member/opportunities" className="mt-5 inline-flex rounded-full bg-kasset-green px-5 py-3 text-sm font-bold text-white">Browse Opportunities</Link></div>
          ...
          <article key={record.slug} className="rounded-[1.5rem] border border-slate-200/80 bg-white p-5 shadow-[0_10px_40px_rgba(15,23,42,0.04)] sm:p-6">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><div><p className="text-xs font-bold uppercase tracking-[0.18em] text-slate-400">{record.campaignCode} • {record.participationId}</p><h2 className="mt-2 text-xl font-semibold tracking-tight text-kasset-ink">{record.propertyName}</h2><p className="mt-1 text-sm text-slate-500">{record.location}</p></div><span className={`inline-flex w-fit rounded-full border px-3 py-1 text-xs font-bold ${statusClasses(record.status)}`}>{record.status}</span></div>
            ...
            <div className="mt-6 flex flex-col gap-3 border-t border-slate-100 pt-5 sm:flex-row sm:items-center sm:justify-between"><p className="text-sm text-slate-500">Latest: <span className="font-semibold text-slate-700">{record.latestUpdate}</span></p><Link href={record.slug.startsWith("par-") || record.slug.length > 20 ? `/member/participations/${record.slug}` : `/member/portfolio/${record.slug}`} className="inline-flex min-h-11 items-center justify-center rounded-full border border-kasset-green px-5 py-2 text-sm font-bold text-kasset-green transition hover:bg-kasset-green hover:text-white">View Details</Link></div>
          </article>
        ))}
      </section>
    </div>
